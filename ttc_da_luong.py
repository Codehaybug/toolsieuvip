...
{vang}[{trang}<{xanh_la}/{trang}>{vang}] {trang_nhat}Tool Name : {vang}Tương Tác Chéo Facebook V1{trang_nhat}
══════════════════════════════════════════════════════
"""
		for _ in logo:
			sys.stdout.write(_)
...
def InputFacebook():
    ...
    while True:
        ...
        log = fb.FilterData
        if log == True:
            idUser = cookieFB.split('c_user=')[1].split(';')[0]
            ...
            ducthinh()
    with open('infoCKFB_Jiray.txt', 'w', encoding='utf-8') as f:
        json.dump(listCK, f)
def InputTTC():
    while True:
        tokenTTC = input(f'{xanh_la_nhat}Nhập {vang}Access Token {xanh_la_nhat}TuongTacCheo : {trang}')
        ttc = TuongTacCheo(tokenTTC)
        log = ttc.Login
        if log[0] == True:
            userttc, xu = log[1], log[2]
            xuNow = cv(xu)
            print(f'{xanh_la_nhat}Đăng Nhập Thành Công TuongTacCheo : {vang}{userttc}{xanh_la_nhat} - Xu Hiện Tại : {vang}{xuNow} {xanh_la_nhat}!')
            with open('infoTTC_Jiray.txt', 'w') as f:
                f.write(tokenTTC)
            break
        else:
...
def ClearCookie(cookie):
    with open('infoCKFB_Jiray.txt', 'r') as f:
        a = json.load(f)
    if cookie in a:
        a.remove(cookie)
    with open('infoCKFB_Jiray.txt', 'w', encoding="utf-8") as f:
...
class FacebookV2:
    ...
    def Get2Fa(self):
        return requests.get(f"https://2fa.live/tok/{self._2fa}").json()["token"]
    @property
    def FilterData(self):
        ...
        try:
            ...
            self.idAcc   = self.cookie.split('c_user=')[1].split(';')[0]
            return True
        except:
            return False
    ...
    def Page(self, idPage):
        ...
        if "IS_SUBSCRIBED" in response.text:
            ...
            return False
class TuongTacCheo:
    def __init__(self, tokenTTC):
        self.tokenTTC = tokenTTC
        self.baseUrl = 'https://tuongtaccheo.com/'
    @property
    def Login(self):
        try:
            data = {
                'access_token': self.tokenTTC
            }
            res = requests.post("https://tuongtaccheo.com/logintoken.php", data= data)
            if 'success' in res.text:
                user = res.json()['data']['user']
                xu   = res.json()['data']['sodu']
                self.headers = {
                    ...
                    'cookie':str(res.headers['Set-Cookie'])
                }
            return True, user, xu
        except:
            return False, f'{do_nhat}Sai Tài Khoản Hoặc Mật Khẩu'
    @property
    def GetXuNow(self):
        data = {
            'access_token': self.tokenTTC
        ...
        res = requests.post("https://tuongtaccheo.com/logintoken.php", data=data)
        return res.json()['data']['sodu']
    def CheckCauHinh(self, idFl):
        data = f'iddat%5B%5D={idFl}&loai=fb'
        response = requests.post(self.baseUrl + 'cauhinh/datnick.php', headers=self.headers, data=data)
        try:
            if response.text == "1":
                return True
            else:
    ...
    def GetJob(self, type:str):
        hd = {
                ...
                'accept-language': 'vi-VN,vi;q=0.9,fr-FR;q=0.8,fr;q=0.7,en-US;q=0.6,en;q=0.5',
                'cookie': str(self.headers['cookie']),
                'priority': 'u=1, i',
        ...
        if type.upper() == "LIKE":return requests.post('https://tuongtaccheo.com/kiemtien/likepostcheo/getpost.php', headers=hd).json()
        elif type.upper() == "LIKE2":return requests.get('https://tuongtaccheo.com/kiemtien/getpost.php', headers=hd).json()
        elif type.upper() == "REACTION":return requests.get('https://tuongtaccheo.com/kiemtien/camxuccheo/getpost.php', headers=hd).json()
        elif type.upper() == "COMMENT":return requests.get('https://tuongtaccheo.com/kiemtien/cmtcheo/getpost.php', headers=hd).json()
        elif type.upper() == "FOLLOW":return requests.get('https://tuongtaccheo.com/kiemtien/subcheo/getpost.php', headers=hd).json()
        elif type.upper() == "SHARE":return requests.get('https://tuongtaccheo.com/kiemtien/sharecheo/getpost.php', headers=hd).json()
        elif type.upper() == "PAGE":return requests.get('https://tuongtaccheo.com/kiemtien/likepagecheo/getpost.php', headers=hd).json()
        elif type.upper() == "GROUP":return requests.get('https://tuongtaccheo.com/kiemtien/thamgianhomcheo/getpost.php', headers=hd).json()
        else:return requests.get('https://tuongtaccheo.com/kiemtien/likepostcheo/getpost.php', headers=hd).json()
    def CollectCoin(self, idJob, type:str):
        global data
        hd = {
            'accept': '*/*',
            'accept-language': 'vi-VN,vi;q=0.9,fr-FR;q=0.8,fr;q=0.7,en-US;q=0.6,en;q=0.5',
        ...
        if type.upper() == "LIKE":url = 'https://tuongtaccheo.com/kiemtien/likepostcheo/nhantien.php'
        elif type.upper() == "LIKE2":url = 'https://tuongtaccheo.com/kiemtien/nhantien.php'
        elif type.upper() == "REACTION":url = 'https://tuongtaccheo.com/kiemtien/camxuccheo/nhantien.php'
        elif type.upper() == "COMMENT":url = 'https://tuongtaccheo.com/kiemtien/cmtcheo/nhantien.php'
        elif type.upper() == "FOLLOW":url = 'https://tuongtaccheo.com/kiemtien/subcheo/nhantien.php'
        elif type.upper() == "SHARE":url = 'https://tuongtaccheo.com/kiemtien/sharecheo/nhantien.php'
        elif type.upper() == "PAGE":url = 'https://tuongtaccheo.com/kiemtien/likepagecheo/nhantien.php'
        elif type.upper() == "GROUP":url = 'https://tuongtaccheo.com/kiemtien/thamgianhomcheo/nhantien.php'
        data = f'id={idJob}'
        ...
        if 'error' in resp.text:
            ...
            msg = resp.json()['mess']
            xuGet = re.findall(r'\d+', str(msg))[0]
            return True, int(self.GetXuNow), xuGet 
banner()
while True:
    if os.path.exists('infoTTC_Jiray.txt'):
        with open('infoTTC_Jiray.txt', 'r') as f:
            line = f.read().split()
        print(line)
        tokenttc = line[0]
        ttc = TuongTacCheo(tokenttc)
        ...
        print(f"{trang}→ {xanh_la_nhat}Nhập Số {hong}[ {trang}1{xanh_la_nhat}{hong} ] {xanh_la_nhat}Để Giữ Lại TTC : {vang}{log[1]}")
        ...
        if int(chon) == 1:ducthinh();break
            ...
            os.remove('infoTTC_Jiray.txt')
        else:
            print(f'{do_nhat}Lựa Chọn Không Hợp Lệ!')
            ducthinh()
    if not os.path.exists('infoTTC_Jiray.txt'):
        open('infoTTC_Jiray.txt', 'a').close()
        InputTTC()
        ducthinh()
        break
while True:
    if os.path.exists('infoCKFB_Jiray.txt'):
        with open('infoCKFB_Jiray.txt', 'r') as f:
            line = json.load(f)
        if len(line) == 0:os.remove('infoCKFB_Jiray.txt');file = open('infoCKFB_Jiray.txt', 'a').close()
        ...
        if int(chon) == 1:
            ...
            os.remove('infoCKFB_Jiray.txt')
        else:
            print(f'{do_nhat}Lựa Chọn Không Hợp Lệ!')
            ducthinh()
    if not os.path.exists('infoCKFB_Jiray.txt'):
        open('infoCKFB_Jiray.txt', 'a').close()
        InputFacebook()
        break
time.sleep(1)
banner()
print(f'{xanh_la_nhat}Nhập Số {nau}[ {trang}1{nau} ] {xanh_la_nhat}Chạy Nhiệm Vụ {vang}LIKE')
print(f'{xanh_la_nhat}Nhập Số {nau}[ {trang}2{nau} ] {xanh_la_nhat}Chạy Nhiệm Vụ {vang}LIKE 2')
print(f'{xanh_la_nhat}Nhập Số {nau}[ {trang}3{nau} ] {xanh_la_nhat}Chạy Nhiệm Vụ {vang}REACTION')
...
while(True):
    with open('infoCKFB_Jiray.txt', 'r') as f:
        ...
        for cookie in linesCookie:
            ...
            ttc.CheckCauHinh(idUser)
            print(f'{xanh_la_nhat}UID : {vang}{idUser} {xanh_la_nhat}<> XU : {vang}{xuNow} {xanh_la_nhat}<> Tổng Cookie : {vang}{str(len(linesCookie))}       ')
            ducthinh()
                ...
                while True:
                    ...
                    if nv == 1:
                        ...
                        if 'error' in get:
                            ...
                            if len(get) == 0:
                                ...
                                print(f'{xanh_la_nhat}Đã Tìm Thấy {vang}{len(get)} {xanh_la_nhat}Nhiệm Vụ LIKE              ', end='\r')
                                for z in get:
                                    if blocked == nvblock:
                                        continue
                                    if doiacc == doinick:
                                    ...
                                    if like:
                                        time.sleep(0.055)
                                        nhan = ttc.CollectCoin(job, 'like')
                                        if nhan[0] == True:
                                            #return True, xuNow, msg#
                                            msg   = '+' +str(nhan[1])
                                            xuNow = nhan[2]
                                            HoanThanh(dem, idjob, 'LIKE', msg, xuNow)
                                            dem += 1
                                            doiacc +=1
                                            blocked +=1
                                        ...
                                        print(f'{do_nhat}LIKE Thất Bại : {job}               ', end='\r')
                                        error+=1
                                        time.sleep(1.5)
                    elif nv == 2:
                        get = ttc.GetJob('like2')
                        if 'error' in get:
                            try:
                                countdown = get['countdown']
                                print(f'{do_nhat}Đang Lấy Nhiệm Vụ LIKE 2, Countdown : {trang_nhat}{countdown}         ', end='\r')
                                time.sleep(1)
                            except:
                                msg = get['error']
                                print(f'{xanh_la_nhat}Đang Lấy Nhiệm Vụ LIKE 2{trang_nhat}{msg}          ', end='\r')
                                time.sleep(1)
                        else:
                            if len(get) == 0:
                                print(f'{do_nhat}Đã Hết Nhiệm Vụ LIKE 2{trang_nhat}          ', end='\r')
                            else:
                                print(f'{xanh_la_nhat}Đã Tìm Thấy {vang}{len(get)} {xanh_la_nhat}Nhiệm Vụ LIKE 2              ', end='\r')
                                for z in get:
                                    if blocked == nvblock:
                                        continue
                                    if doiacc == doinick:continue
                                    job = z['idpost']
                                    idjob = job.split('_')[1] if '_' in job else job
                                    like = fb.Reaction('LIKE', idjob)
                                    if like:
                                        time.sleep(0.055)
                                        nhan = ttc.CollectCoin(job, 'like2')
                                        if nhan[0] == True:
                                            #return True, xuNow, msg#
                                            xuNow = nhan[1]
                                            msg   = '+' +str(nhan[2])
                                            HoanThanh(dem, idjob, 'LIKE 2', msg, xuNow)
                                            dem += 1
                                            blocked +=1
                                            doiacc+=1
                                            nghingoi(delaymin, delaymax)
                                        else:
                                            print(f'{do_nhat}Nhận Thất Bại : {job} - LIKE 2              ', end='\r')
                                            time.sleep(1.5)
                                    else:
                                        print(f'{do_nhat}LIKE Thất Bại : {job}               ', end='\r')
                                        error+=1
                        ...
                        get = ttc.GetJob('reaction')
                        if 'error' in get:
                            try:
                                countdown = get['countdown']
                                print(f'{do_nhat}Đang Lấy Nhiệm Vụ REACTION, Countdown : {trang_nhat}{countdown}         ', end='\r')
                                time.sleep(1)
                            except:
                                msg = get['error']
                            ...
                            if len(get) == 0:
                                ...
                                for z in get:
                                    ...
                                    if like:
                                        time.sleep(0.055)
                                        nhan = ttc.CollectCoin(job, 'REACTION')
                                        if nhan[0] == True:
                                            #return True, xuNow, msg#
                                            xuNow = nhan[1]
                                            msg   = '+' +str(nhan[2])
                                        ...
                                        print(f'{do_nhat}REACTION Thất Bại : {job}               ', end='\r')
                                        error+=1
                                        time.sleep(1.5)
                    elif nv == 4:
                        get = ttc.GetJob('comment')
                        if 'error' in get:
                            try:
                                countdown = get['countdown']
                            ...
                            if len(get) == 0:
                                ...
                                for z in get:
                                    ...
                                    job = z['idpost']
                                    a = z['nd']
                                    msg = a.split('["')[1].split('"')[0]
                                    idjob = job.split('_')[1] if '_' in job else job
                                    like = fb.Comment(idjob, msg)
                                    if like:
                                        time.sleep(1)
                                        nhan = ttc.CollectCoin(job, 'comment')
                                        if nhan[0] == True:
                                            #return True, xuNow, msg#
                                            xuNow = nhan[1]
                                            msg   = '+' +str(nhan[2])
                                        ...
                                        print(f'{do_nhat}COMMENT Thất Bại : {job}               ', end='\r')
                                        error+=1
                                        time.sleep(1.5)
                    elif nv == 5:
                        get = ttc.GetJob('follow')
                        if 'error' in get:
                            try:
                                countdown = get['countdown']
                            ...
                            if len(get) == 0:
                                ...
                                for z in get:
                                    ...
                                    if like:
                                        time.sleep(0.055)
                                        nhan = ttc.CollectCoin(job, 'follow')
                                        if nhan[0] == True:
                                            #return True, xuNow, msg#
                                            xuNow = nhan[1]
                                            msg   = '+' +str(nhan[2])
                        ...
                        get = ttc.GetJob('share')
                        if 'error' in get:
                            try:
                                countdown = get['countdown']
                                print(f'{do_nhat}Đang Lấy Nhiệm Vụ SHARE, Countdown : {trang_nhat}{countdown}         ', end='\r')
                                time.sleep(1)
                            except:
                                msg = get['error']
                            ...
                            if len(get) == 0:
                                ...
                                for z in get:
                                    ...
                                    if like:
                                        time.sleep(0.055)
                                        nhan = ttc.CollectCoin(job, 'share')
                                        if nhan[0] == True:
                                            #return True, xuNow, msg#
                                            xuNow = nhan[1]
                                            msg   = '+' +str(nhan[2])
                                        ...
                                        print(f'{do_nhat}SHARE Thất Bại : {job}               ', end='\r')
                                        error+=1
                                        time.sleep(1.5)
                    elif nv == 7:
                        get = ttc.GetJob('group')
                        if 'error' in get:
                            try:
                                countdown = get['countdown']
                                print(f'{do_nhat}Đang Lấy Nhiệm Vụ GROUP, Countdown : {trang_nhat}{countdown}         ', end='\r')
                                time.sleep(1)
                            except:
                                msg = get['error']
                            ...
                            if len(get) == 0:
                                ...
                                for z in get:
                                    ...
                                    if like:
                                        time.sleep(0.055)
                                        nhan = ttc.CollectCoin(job, 'group')
                                        if nhan[0] == True:
                                            #return True, xuNow, msg#
                                            xuNow = nhan[1]
                                            msg   = '+' +str(nhan[2])
                                        ...
                                        print(f'{do_nhat}GROUP Thất Bại : {job}               ', end='\r')
                                        error+=1
                                        time.sleep(1.5)
                    elif nv == 8:
                        get = ttc.GetJob('page')
                        if 'error' in get:
                            try:
                                countdown = get['countdown']
                                print(f'{do_nhat}Đang Lấy Nhiệm Vụ PAGE, Countdown : {trang_nhat}{countdown}         ', end='\r')
                                time.sleep(1)
                            except:
                                msg = get['error']
                            ...
                            if len(get) == 0:
                                ...
                                for z in get:
                                    ...
                                    if like:
                                        ...
                                        print(f'{do_nhat}PAGE Thất Bại : {job}               ', end='\r')
                                        error+=1
                                        time.sleep(1.5)
                    else:
                        pass
                    ...
                    if error == 50:
                        ...
                        if check:
                            ...
                            print(f'{do_nhat}Tài khoản UID : {vang}{idUser}{do_nhat} Đã Bị Die Cookie                ', end='\r')
                            ClearCookie(cookie)
                            ducthinh()
                    with open('infoCKFB_Jiray.txt', 'r') as f:
                        linesCookie = json.load(f)
